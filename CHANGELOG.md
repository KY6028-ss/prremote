...
- `prremote list` labelled every Linux port `unknown` when the USB device did not sit exactly three levels above the tty node in sysfs. `Detector#port_label` read `/sys/class/tty/<port>/device/../../../idVendor`, a fixed depth that does not hold across host shapes: a ttyUSB bridge adds a level the native-CDC ttyACM nodes do not, and under WSL the chain hangs off `vhci_hcd` rather than a PCI controller. The lookup now walks up from the tty's device node until it finds `idVendor`. _Reproduced with a Pico W attached to WSL through usbipd-win, where the vendor file sits one level above the interface and the board was reported as `(unknown)` despite `idVendor` reading `2e8a`._
- Nothing worked on Windows past opening the port: `Serial#write` failed with `ERROR_SEM_TIMEOUT` and the board never answered. rubyserial leaves DTR deasserted there, and a Pico's TinyUSB CDC only starts transmitting once the host raises it. Ports are now opened through `Prremote::SerialPort.open`, which raises DTR and RTS via `EscapeCommFunction` on Windows and is a plain `Serial.new` everywhere else. _Confirmed by driving the same port from .NET's `SerialPort`: with `DtrEnable` the board replies `READY prremote-runtime/0.3.2`, without it the write reports success and nothing ever comes back._
- `prremote list` on Windows listed mangled port names and could not tell a board from a Bluetooth virtual port, so auto-detection picked the wrong one and every command needed an explicit `--port`. Two causes: `Detector#serial_ports` read `HARDWARE\DEVICEMAP\SERIALCOMM`, whose values some drivers store without a NUL terminator — Ruby's registry reader chops the last character off those, turning `COM3` into `COM`; and `port_label` had no Windows branch at all, so every port came back `unknown` and `find_device` fell through to the first candidate. Ports now come from `SYSTEM\CurrentControlSet\Enum\USB`, which carries both the assigned `PortName` and the VID the label needs, with SERIALCOMM kept as a secondary source filtered to well-formed `COM<number>` names. _Verified against a Pico W on COM7 alongside two Bluetooth ports: `list` now reports `COM7  (Pico (prremote/R2P2))` and `find_device` returns it._
